...
2.1 Explanatory Variable Analysis: Impact of Physical Factors on Used Car Prices 2.1.1 Histograms of Price Distributions
...
The price distribution charts for each brand show a variety of price ranges, reflecting the diversity of models and conditions for each vehicle. Here are the conclusions based on descriptive statistics:

Volvo: Average price CHF 39,811, with a wide range from CHF 5,500 to CHF 94,900.
The price distribution for Volvo shows a relatively wide spread, indicating a diverse range of prices. This suggests that Volvo offers models catering to various market segments.


Toyota: Average price CHF 14,872, ranging from CHF 400 to CHF 80,000.
Toyota's price distribution appears to be slightly skewed towards the lower end, suggesting a larger number of more affordable models. This is consistent with Toyota's reputation for offering budget-friendly and reliable vehicles.


Skoda : Average price CHF 18,961, ranging from CHF 300 to CHF 59,900.
Skoda's price distribution is somewhat similar to Toyota, with a concentration in the lower to mid-price range. This indicates that Skoda also targets the budget and mid-range market.


Golf : Average price CHF 19,398, ranging from CHF 350 to CHF 59,990.
The Golf's price distribution is quite broad, reflecting a diverse range of models with varying prices. This indicates that the Golf lineup caters to a wide range of customers, from budget-conscious to more premium buyers.


Audi: average price CHF 20.928, ranging from CHF 600 to CHF 66.500.
Audi's price distribution is more spread out, with a significant presence in the higher price range. This is in line with Audi's position as a manufacturer of premium vehicles.


These data show significant differences between brands in terms of average price and price range.
...
graph <- ggplot(combined_df, aes(x = date, y = price, fill = Brand)) +
  ...
  labs(title = "Prix moyen par marque et année", x = "Année", y = "Prix moyen") +
  theme_minimal()
...
graph0 <- ggplot(all_cars, aes(x = body.type, y = price, fill = brand)) +
  geom_boxplot() +
  labs(title = "Distribution des prix par type de carrosserie et marque",
       x = "Type de carrosserie", y = "Prix") +
  theme_minimal() +
...
The relationship between a car's mileage and its selling price is a classic indicator of value in the used car market. The following scatter plots illustrate this relationship for each brand
...
From the plots, it's evident that as mileage increases, the price of the car tends to decrease. This trend is consistent across all brands, underlining the fundamental role of mileage in determining a car's market value.

2.1.6 Engine Power (Power) and Price Correlation

The engine power of a car, a significant performance metric, also plays a crucial role in pricing. Scatter plots were created for each brand to observe this correlation
...
The data suggests a positive correlation between engine power and car price. Higher-powered cars, often offering better performance, command higher prices. This trend is particularly pronounced in brands known for their performance-oriented models.

2.1.7 Influence of Fuel Type on Car Pricing 

Different fuel types cater to various market segments and preferences. To understand how fuel type influences car prices, average prices for different fuel types were plotted in bar charts for each brand
...
plot_audi <- create_bar_plot(audi, "AUDI - Type de Carburant vs Prix Moyen")
plot_golf <- create_bar_plot(golf, "GOLF - Type de Carburant vs Prix Moyen")
plot_toyota <- create_bar_plot(toyota, "TOYOTA - Type de Carburant vs Prix Moyen")
plot_skoda <- create_bar_plot(skoda, "SKODA - Type de Carburant vs Prix Moyen")
plot_volvo <- create_bar_plot(volvo, "VOLVO - Type de Carburant vs Prix Moyen")
