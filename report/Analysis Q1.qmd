...
In the graph above for the VW Golf, we can see that the average price goes up after a vehicle age of approx. 9000 days (= 24 years, 7 months, 21 days) which is counter-intuitive.
...
We still notice quite a high standard error for all vehicles, which decreases the reliability of the predictions and the usefulness of the tool

For the 5 car models that we have researched, the R-Squared values are between 0.90 and 0.94, which we consider to be very good.

```{r}

#plotly bar chart with VIFs for each factor for each vehicle

vif_values <- vif(golf_model3)
vif_values
...
plot_data2 <- data.frame(Variables = c("kilometers", "kilometers.squared", "vehicle.age", "vehicle.age.squared", "power", "expertise", "warranty", "wagon", "diesel", "manual", "awd"), VIF = 5)
