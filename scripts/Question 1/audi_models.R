...
summary(lm(price ~ wagon)) # NOT Significant
...
# They are all significant except diesel. Let's add them to our original model.
...
# We keep diesel as it is significant in model4, but we remove hybrid.

model4 <- lm(price ~ vehicle.age + vehicle.age.squared + kilometers + kilometers.squared + power + expertise + warranty + diesel + manual + awd)
summary(model4)

# All our variables are now significant. Our adjusted R-squared is 0.9154, which is the proportion of the variance of the price that is explained by our model. We consider this to be a good score. Let's check the residuals vs fitted plot and the standard deviation of the residuals.
